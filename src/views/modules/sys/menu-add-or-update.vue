  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item v-if="dataForm.type === 1" label="菜单路由">
        <el-input v-model="dataForm.url" placeholder="菜单路由如:sys/menu"></el-input>
      </el-form-item>
      <el-form-item v-if="dataForm.type !== 0" label="授权标识" prop="perms">
        <el-input v-model="dataForm.perms" placeholder="多个用逗号分隔, 如: user:list,user:create"></el-input>
      </el-form-item>
      <el-form-item v-if="dataForm.type !== 0" label="排序号" prop="orderNum">
...
export default {
  ...
  methods: {
    init(id) {
      ...
      this.$http({
        url: this.$http.adornUrl("/menu/select", "CQ"),
        method: "post",
        params: this.$http.adornParams()
      })
        .then(({ data }) => {
          if (data.code == 200) {
            var list = treeDataTranslate(data.data, "menuId");
            console.log(list);
            this.menuList = list;
          }
        })
        ...
        .then(() => {
          if (!this.dataForm.id) {
            ...
            // 修改
            this.$http({
              url: this.$http.adornUrl(`/menu/info`, "CQ"),
              method: "post",
              data: {
                permissionId: this.dataForm.id
              }
            }).then(({ data }) => {
              ...
              this.dataForm.perms = data.data.permission;
              this.dataForm.orderNum = data.data.ranking;
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/menu/${!this.dataForm.id ? "save" : "update"}`,
              "CQ"
            ),
            method: "post",
            data: {
              permissionInfo: JSON.stringify({
                name: this.dataForm.name,
                type: this.dataForm.type,
                parentid: this.dataForm.parentId,
                url: this.dataForm.url,
                icon: this.dataForm.icon,
                permission: this.dataForm.perms,
                status: 1,
                remark: "",
                ranking: this.dataForm.orderNum,
                id: this.dataForm.id || ""
              })
            }
          }).then(({ data }) => {
